<template>
	...
	>
		<!---Type dialog--->
		<v-dialog v-model="dialogType" persistent max-width="500" scrollable>
			<v-form ref="form1" v-model="validNewIngredient" lazy-validation>
				<v-card>
					<v-card-title> 
            <v-container fluid>
              <v-row class="mb-2">
									...
									<template v-slot:no-data>
										...
										<v-list-item v-else>
											<span class="subheading mr-2"
												>Veuillez entrer un ingrédient</span
											>
										</v-list-item>
									</template>
									<template
							...
							<v-col cols="12" sm="4" md="4" lg="4" xl="4">
								...
								>
									<template v-slot:item="props">
										<v-icon
											:color="
												props.isFilled ? 'green lighten-3' : 'grey lighten-1'
											"
											large
											@click="props.click"
										>
	...
	export default {
		...
		data() {
			return {
				...
				items: [{ header: "Rechercher votre ingrédient" }],
				nonce: 1,
		...
		methods: {
			...
			async addIngredient() {
				...
				let validProduct;
				this.openFoodFactSelectedItem === null
					? ((this.infoMessageSelectProduct = true), (validProduct = false))
					: ((this.infoMessageSelectProduct = false), (validProduct = true));
			...
			callOpenFoodFact() {
				...
				DAODenree.getOpenFoodFactProduct(this.newIngredient, this.typeChoix).then(
					(response) => {
						this.openFoodFactList = response.data;
		...
		watch: {
			dialogNewRecipeProps() {
				...
				if (this.dialogNewRecipeProps) {
					DAOTag.getAll().then((response) => {
						this.tagsListe = response.data;
					});

					DAOCategorie.getAll().then((response) => {
			...
			search() {
				if (this.search !== null && this.search.length === 3) {
					DAODenree.searchProduct(this.search).then((response) => {
